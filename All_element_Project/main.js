...
// uniform 变量
var uniformLoc = {
    objectTransformMatrix: null,    //模型变换矩阵
    modelMatrix: null,  //建模
    modelViewMatrix: null,  //模型视图矩阵
    projectionMatrix: null, //透视矩阵
    ambientProduct:null,    //环境光
    diffuseProduct:null,    //漫反射光
    specularProduct:null,   //镜面反射光
    shininess: null //高光度
};
...
var camera = {
    ...
    up:[0, 1, 0],
    speed:[0, 0, 0],
    fovy: 55, aspect: 1, near: 0.5, far: 100, left:-4, right:4, bottom:-4, top:4,
    ortho: false,  // 是否正交投影
...
// 创建墨西哥帽
function createhat(origin = [0,0,0], material = new Material(), hasTexture = false){
    var nRows = 50;
    var nColumns = 50;
    var data = new Array(nRows);

    var hat = new RenderObject(translate(origin), [], [], [], material);
    ...
    for(var i=0; i<nRows-1; i++) {
        for(var j=0; j<nColumns-1;j++) {
            hat.normalsArray.push( vec4(2*i/nRows-1, data[i][j], 2*j/nColumns-1, 1.0));
            hat.normalsArray.push( vec4(2*(i+1)/nRows-1, data[i+1][j], 2*j/nColumns-1, 1.0));
            hat.normalsArray.push( vec4(2*(i+1)/nRows-1, data[i+1][j+1], 2*(j+1)/nColumns-1, 1.0));
            hat.normalsArray.push( vec4(2*i/nRows-1, data[i][j+1], 2*(j+1)/nColumns-1, 1.0) );

            hat.pointsArray.push( vec4(2*i/nRows-1, data[i][j], 2*j/nColumns-1, 1.0));
            hat.pointsArray.push( vec4(2*(i+1)/nRows-1, data[i+1][j], 2*j/nColumns-1, 1.0));
            hat.pointsArray.push( vec4(2*(i+1)/nRows-1, data[i+1][j+1], 2*(j+1)/nColumns-1, 1.0));
            hat.pointsArray.push( vec4(2*i/nRows-1, data[i][j+1], 2*(j+1)/nColumns-1, 1.0) );
        }
    }
    return hat;
}
...
function createObjects(){
    var sphere = createSphere([0, 0, 0]);
    //sphere.selfRotating = new Rotation([0,-1,0], 1); // 自转
    sphere.material.ambient = vec3(0.0, 0.15, 0.2);
    sphere.material.diffuse = vec3(0.0, 0.15, 0.2);
    ...
    var cube = createCube([0,-0.9,0], cubeMaterial);
    cube.selfRotating = new Rotation([0,1,0], 1); // 自转
    cube.transform = mult(scalem(2.5,2.5,2.5), cube.transform);

    var hat = createhat([0,1.0,0]);
    //hat.selfRotating = new Rotation([0,1,0], 1); // 自转

    transparent_objects.push(hat);
    opaque_objects.push(cube);
    opaque_objects.push(sphere);
    opaque_objects.push(hat);
    cubeCtr.obj = cube;   
}
...
var vBuffer;    //存放顶点坐标的buffer，由createBuffer得到。
var nBuffer;    //存放顶点法向量的buffer，由createBuffer得到。
var tBuffer;    //存放纹理坐标的buffer，由createBuffer得到。
//////////////////////////////////////////////////////////////////////////////////////////
